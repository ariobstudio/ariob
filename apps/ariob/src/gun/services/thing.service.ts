...
export const createThingService = <T extends Thing, TSchema extends z.ZodType<T>>(
  schema: TSchema,
  soulPrefix: string
) => {
  // Create a new thing
  const create = async (data: Omit<T, 'id' | 'createdAt' | 'soul' | 'version'>): Promise<T> => {
  ...
  // Update a thing
  const update = async (id: string, updates: Partial<Omit<T, 'id' | 'createdAt' | 'soul' | 'version'>>): Promise<T | null> => {
  ...
  const list = async (): Promise<T[]> => {
    return new Promise((resolve) => {
      ...
      gun.get(soulPrefix).map().once((data: any, key: string) => {
  ...
  const subscribe = (id: string, callback: (data: T | null) => void): () => void => {
