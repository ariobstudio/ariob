 ...
 *
 * @param initialFeature - Feature to show initially (defaults to 'feed')
 *
 * @example
 * ```typescript
 * const navigator = useNavigator('feed');
 *
 ...
 *
 * // Navigate to thread
 * navigator.navigate('thread', { threadId: 'abc123' });
 *
 * // Go back
 ...
 * ```
 */
export function useNavigator(initialFeature: FeatureName = 'feed'): Navigator {
