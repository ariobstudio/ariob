...
export const Profile = ({ data, onAnchor }: ProfileProps) => {
  ...
  const handleAnchor = async () => {
    ...
    if (!raw || raw.length < 2 || !onAnchor) return;
    setLoading(true);
    await onAnchor(raw);
    setLoading(false);
  };
      ...
      <View style={styles.container}>
        ...
        <View style={styles.content}>
          ...
          <View style={styles.inputContainer}>
            ...
            />
            <Pressable
              style={[styles.anchorButton, (!canAnchor || loading) && styles.anchorButtonDisabled]}
              onPress={handleAnchor}
              disabled={!canAnchor || loading}
            >
              {loading ? (
      ...
      <View style={styles.content}>
        ...
        {data.stats ? (
          <View style={styles.statsRow}>
            ...
            <View style={styles.stat}>
              <Text style={styles.statValue}>{data.stats.followers}</Text>
              <Text style={styles.statLabel}>Followers</Text>
            </View>
            <View style={styles.stat}>
              <Text style={styles.statValue}>{data.stats.following}</Text>
