...
crypto.subtle.importKey = (format, keyData, algorithm, extractable, usages) => {
  ...
  console.log('---> [input] importKey <format =', format, 'param =', param, 'algorithm =', algorithm, 'extractable =', extractable, 'usages =', usages, '>')
  return maybePromise(
...
crypto.subtle.deriveBits = (algorithm, baseKey, length) => {
  ...
  console.log('---> [input] deriveBits <alg =', alg, 'baseKey =', baseKey, 'length =', length, '>')
  return maybePromise(
...
crypto.subtle.deriveKey = (algorithm, baseKey, derivedKeyAlgorithm, extractable, usages) => {
  ...
  console.log('---> [input] deriveKey <alg =', alg, 'baseKey =', baseKey, 'dAlg =', dAlg, 'extractable =', extractable, 'usages =', usages, '>')
  return maybePromise(
...
crypto.getRandomValues = (typedArray) => {
  ...
  console.log('---> [input] getRandomValues <length =', length, '>')
  return maybePromise(native.getRandomValues(length)).then((b) => {
    console.log('---> [output] getRandomValues <b =', b, '>')
    if (!b) throw new Error('getRandomValues failed');
    typedArray.set(toU8(b));
    return typedArray;
  });
};
...
globalThis.btoa = (input) => {
  ...
  console.log('---> [input] btoa <str =', str, '>')
