(() => {
  ...
  USE((module) => {
    ...
    function spot(word, sorted, parse) {
      parse = parse || spot.no || (spot.no = (t) => t); // TODO: BUG???? Why is there substring()||0 ? // TODO: PERF!!! .toString() is +33% faster, can we combine it with the export?
      var L = sorted,
        min = 0,
        page,
        found,
    ...
    function heal(l, s) {
      ...
      if (
        (e =
          i +
          2 +
          Number.parseInt((e = l[i + 1]).substring(0, e.indexOf('"')) || e)) !=
        e
      ) {
        return [];
    ...
    function sort(p, l) {
      var f = (p.from =
        'string' == typeof p.from ? slot(p.from) : p.from || []);
      if (!(l = l || p.limbo)) {
        return f;
      }
      return mix(p).sort((a, b) =>
        (a.word || B.decode('' + a)) < (b.word || B.decode('' + b)) ? -1 : 1,
      );
    }
    function mix(p, l) {
    ...
    function decord(t) {
      var o = {},
        i = t.lastIndexOf('%'),
        c = Number.parseFloat(t.slice(i + 1));
  ...
  USE((module) => {
    ...
    var random = String.random || (() => Math.random().toString(36).slice(2));
  })(USE, './ask');
  USE((module) => {
    function Gun(o) {
      if (o instanceof Gun) {
  ...
  USE((module) => {
    ...
    Gun.chain.chain = function (sub) {
      var at = this._,
        chain = new (sub || this).constructor(this),
        cat = chain._,
        root;
    ...
    function input(msg, cat) {
      ...
      var root = cat.root,
        ...
        state = tmp['>'] || Number.NEGATIVE_INFINITY,
    ...
    var empty = {},
      ...
      text_rand = String.random,
      valid = Gun.valid,
      obj_has = (o, k) => o && Object.prototype.hasOwnProperty.call(o, k),
      state = Gun.state,
      state_is = state.is,
      state_ify = state.ify;
  })(USE, './chain');
